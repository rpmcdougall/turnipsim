...
| 1" rule on charge | Charge end-cell must be >1" from any non-target unit. If every cell next to the target is occupied or within 1" of a third unit, the charge fails — but the charger still moves (see "Failed charge movement"). | Charge succeeds with end-cell within 1" of a non-target friendly or enemy |
| Failed charge movement | If the target is out of `M + move_bonus` range, or every 8-ring cell next to the target is blocked (occupied / objective / 1"-rule), the charger still advances its full charge distance toward target via the shortest legal route. Action log shows `charge_failed` with a `failure_reason` (`out_of_range`, `no_legal_adjacent_cell`, `adjacent_cell_unreachable`). No panic test, no Stand and Shoot, no melee. | Charge silently fails with no movement; or charger ends within 1" of a non-target unit; or melee/panic/S&S still resolves for a charge that didn't make contact |
